  ...
  render() {
    ...
    const descriptions = (
      <div>
        ...
        (<div className="explanation">
          <span><span className="icon icon-additive" /> {this.politicalLevelText(post.political_quintile)}</span>
        </div>)
        }

        {(post.toxicity !== null) && (post.toxicity !== -1) &&
        (<div className="explanation">
          <span><span className="icon icon-toxicity" /> This post is {rudenessScoreToString(post.toxicity)}</span>
        </div>)
        }
        {(post.is_corporate !== null) &&
        (<div className="explanation">
          <span><span className="icon icon-corporate" /> {(post.is_corporate) ? 'Posted' : 'Not Posted'} by a brand</span>
        </div>)
        }
        {(post.news_score !== null) &&
        (<div className="explanation">
          <span><span className="icon icon-seriousness" /> This post is {seriousnessScoreToString(post.news_score)}</span>

        </div>)
        }
        {(post.virality_count !== null) &&
        (<div className="explanation">
          <span><span className="icon icon-virality" /> This post is {this.viralityScoreToString(post.virality_count)}</span>
        </div>)
        }
        {hasKeywordRule &&
        (<div className="explanation">
          <span><span className="icon icon-seriousness" /> This post {this.keywordRulesText()}</span>
        </div>)
        }
        {hasAdditiveRule &&
        (<div className="explanation">
          <span><span className="icon icon-additive" /> This post {this.additiveRulesText()}</span>
        </div>)
        }
      </div>
    );
